...
{
    public class SuperAdminQueryHandler
    {
        private readonly string connectionString = "datasource=localhost;port=3306;username=superadmin;password=superadmin;" +
           "database=restaurantmanagement;";

        public SuperAdminQueryHandler()
        {
        ...
        public bool insertInventoryItem(uint quantity, string batchNum, DateTime expiryDate, string shelfLoc, uint menuID)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    using (MySqlCommand q = new MySqlCommand("INSERT INTO `inventory_item`(`Quantity`, " +
                        ...
                        "(@quantity, @batchNum, @expiryDate, @shelfLoc, @menuID, NULL)", connection))
                    {
        ...
        public bool updateInventoryItem(uint quantity, string batchNum, DateTime expiryDate, string shelfLoc, uint menuID, ulong inventoryID)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    using (MySqlCommand q = new MySqlCommand("UPDATE `inventory_item` SET `Quantity`=@quantity, " +
                        ...
                        "`Menu_ID`=@menuID, `M_Em_ID`=NULL WHERE Inventory_Item_ID = @inventoryID", connection))
                    {
                        q.Parameters.AddWithValue("@quantity", quantity);
                        q.Parameters.AddWithValue("@batchNum", batchNum);
                        q.Parameters.AddWithValue("@expiryDate", expiryDate);
        ...
        {
            if (role != "cashier" && role != "chef" && role != "manager" && role != "superadmin") return false;
            try
            {
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
        ...
        {
            if (role != "cashier" && role != "chef" && role != "manager" && role != "superadmin") return false;
            if (oldRole != "cashier" && oldRole != "chef" && oldRole != "manager" && oldRole != "superadmin") return false;
            try
            {
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        {
                            using (MySqlCommand q = new MySqlCommand("DELETE FROM `superadmin` WHERE S_Em_ID = @sid", connection))
                            {
                                q.Parameters.AddWithValue("@sid", employeeID);
                                q.ExecuteNonQuery();
                            }
                        }
                        using (MySqlCommand q = new MySqlCommand($"INSERT INTO `{role}` VALUES (@id)", connection))
